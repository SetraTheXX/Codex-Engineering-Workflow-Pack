...
if (isReviewer) {
  ...
  fs.writeFileSync(
    eventPath,
    JSON.stringify({ role, event: "fake-reviewer-completed", decision: "PASS" }) + "\\n",
  );

  if (lastMessage) {
    fs.mkdirSync(path.dirname(lastMessage), { recursive: true });
    fs.writeFileSync(lastMessage, "Fake reviewer completed with Decision: PASS.\\n");
  }

  console.log("fake codex stdout: reviewer PASS");
  console.error("fake codex stderr: reviewer PASS");
  process.exit(0);
}
